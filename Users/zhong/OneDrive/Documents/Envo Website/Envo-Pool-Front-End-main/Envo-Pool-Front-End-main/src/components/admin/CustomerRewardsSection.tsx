...
import { Gift, Copy, Loader2, Check, Trash2, Eye, EyeOff, Sparkles } from "lucide-react";
...
export default function CustomerRewardsSection({ userId }: { userId: string }) {
  ...
  const customer: any = useMemo(
    () => (customers || []).find((c: any) => (c.user_id || c.userId || c._id || c.id) === userId),
    [customers, userId],
  );
  const currentPoints = Math.max(0, Math.floor(Number(customer?.rewardPoints ?? customer?.reward_points ?? 0)));
  const lifetimePoints = Math.max(0, Math.floor(Number(customer?.lifetimePoints ?? customer?.lifetime_points ?? currentPoints)));
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-3">
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <Sparkles className="h-3 w-3 text-amber-400" /> Current Points
            </div>
            <p className="text-xl font-bold text-amber-400 mt-1">{currentPoints.toLocaleString()}</p>
          </div>
          <div className="rounded-lg border border-border bg-card p-3">
            <p className="text-xs text-muted-foreground">Lifetime Points</p>
            <p className="text-xl font-bold mt-1">{lifetimePoints.toLocaleString()}</p>
          </div>
        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2 text-right whitespace-nowrap">
                        {!deleted && !r.redeemed && r.tangible && (
                          <Button
                            size="sm"
                            variant="outline"
                            className="h-7 mr-1"
                            onClick={(e) => { e.stopPropagation(); markRedeemed.mutate(r._id || r.id); }}
                            disabled={markRedeemed.isPending}
                            title="Mark redeemed"
                          >
                            <Check className="h-3.5 w-3.5 mr-1" /> Redeemed
                          </Button>
                        )}
                        {!deleted && !r.redeemed && (
                          <Button
