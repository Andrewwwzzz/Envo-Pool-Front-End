...
function MembershipCard({
  ...
  const endDate = membership?.endDate ?? membership?.renewalDate;
  const endDatePassed = endDate ? new Date(endDate).getTime() < Date.now() : false;
  const isExpired =
    status === "expired" ||
    status === "cancelled" ||
    (status !== "active" && !membership?.active && endDatePassed);
  const isActive = !isExpired && (status === "active" || membership?.active);
  ...
  return (
    <Card className={`card-premium ${dimmed}`}>
      ...
      <CardContent className="space-y-4 text-sm">
        ...
        {/* Monthly hours progress — only for non-private active memberships with a threshold set */}
        {isActive && hoursEntry && !plan?.isPrivate && hoursEntry.requiredHours > 0 && (() => {
...
export default function DashboardMembership() {
  ...
  const handleLockerPurchase = async () => {
    ...
    try {
      ...
      toast.success(`Locker #${body.rental?.lockerNumber ?? ""} assigned! Your PIN will be shown in the locker section.`);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Locker purchase confirm dialog */}
      <AlertDialog open={!!lockerPinPlan} onOpenChange={(o) => !o && setLockerPinPlan(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              A locker will be randomly assigned to you. Your PIN will be pre-set by staff and shown in
              your dashboard after purchase.
              <br /><br />
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={lockerPurchasing}>Cancel</AlertDialogCancel>
            <AlertDialogAction
              onClick={(e) => { e.preventDefault(); handleLockerPurchase(); }}
              disabled={lockerPurchasing}
            >
